...
function sanitizeUser(user: { id: number; name: string; email: string; role?: string; approvalStatus?: string }) {
  return {
    id: user.id,
    name: user.name,
    email: user.email,
...
async function sendApprovalRequestEmail(user: { name: string; email: string; approvalToken: string }) {
...
async function getExistingResolution(ticketId: number) {
...
async function resolveAndPersistTicket(ticket: Ticket) {
...
function toIntegrationResponse(result: Awaited<ReturnType<typeof resolveAndPersistTicket>>, metadata: Record<string, unknown> = {}) {
  return {
    ...metadata,
...
async function resolveTicketById(ticketId: number) {
