...
function hasSkill(employee: Employee, skill: string) {
...
function findEmployee(employees: Employee[], options: { name?: string; department?: string; skill?: string }) {
  return (
    employees.find((employee) => options.name && normalize(employee.name) === normalize(options.name)) ??
...
function baseValidationSteps(ticket: Ticket): ResolutionStep[] {
  return [
    {
    ...
    },
    {
      ...
      detail:
        ticket.hubOrderNo && ticket.materialNo
    ...
    },
    {
      ...
      detail:
        !ticket.hasUserStatusBlock && Boolean(ticket.shippingType) && ticket.autoObdRelease === true
...
function makeDecision(
  ...
  options: {
    ...
    reason: string;
    score: number;
    assignee?: { name?: string; department?: string; skill?: string };
    steps: ResolutionStep[];
  }
): ResolutionDecision {
...
export function resolveTicket(ticket: Ticket, employees: Employee[]): ResolutionDecision {
