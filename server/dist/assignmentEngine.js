function hasSkill(employee, requiredSkill) {
    return employee.skills
        .split(",")
        .map((skill) => skill.trim().toLowerCase())
        .includes(requiredSkill.toLowerCase());
}
function workloadBonus(employee, candidates) {
...
export function assignTicket(ticket, employees) {
    const departmentMatches = employees.filter((employee) => employee.department === ticket.department);
    const availableInDepartment = departmentMatches.filter((employee) => employee.availability && employee.activeTickets < employee.maxCapacity);
    ...
    const baseSteps = [
        {
            ...
            detail: departmentMatches.length > 0
        ...
        },
        {
            ...
            detail: exactSkillMatches.length > 0
        ...
        },
        {
            ...
            detail: availableInDepartment.length > 0
